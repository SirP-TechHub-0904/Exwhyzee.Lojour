...
namespace Exwhyzee.Lojour.Data.Repository.BarnerImages
{
    ...
    {
        #region constants
        private const string CACHE_BARNER = "Exwhyzee.Wimbig.Data.Barner";
            ...
            try
            {
               if (img == null)
                throw new ArgumentNullException(nameof(img),"Model cannot be null");
                   ...
                   _signal.SignalToken(CACHE_BARNER);
        ...
        public async Task<BarnerFile> GetById(long Id)
        {
            ...
            string cacheKey = $"{CACHE_BARNER}.getById.{Id}";
            var image = _memoryCache.GetOrCreate(cacheKey, (entry) => {

                entry.ExpirationTokens.Add(_signal.GetToken(CACHE_BARNER));
        ...
        {
            string cacheKey = $"{CACHE_BARNER}.getAllBarner";
