...
Create PROCEDURE [dbo].[spListAllTrainingAndWorkShop]	
...
AS
	...
	SELECT *
	INTO #TempTableFiltered FROM [TrainingAndWorkShop] 
...
AND (@userProfileId IS NULL OR UserProfileId = @userProfileId)
	...
	--set table total
	SELECT @TotalCount = ISNULL(COUNT(Id),0) FROM [TrainingAndWorkShop] WITH(NOLOCK) 
