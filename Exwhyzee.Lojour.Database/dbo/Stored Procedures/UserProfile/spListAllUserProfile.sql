...
Create PROCEDURE [dbo].[spListAllUserProfile]	
...
AS
	...
	SELECT *
	INTO #TempTableFiltered FROM [UserProfile] 

	
	WHERE 1 = 1 
	AND (@searchString IS NULL OR [UserProfile].Title Like @searchString)

	--set table total
	SELECT @TotalCount = ISNULL(COUNT(Id),0) FROM [UserProfile] WITH(NOLOCK) 
