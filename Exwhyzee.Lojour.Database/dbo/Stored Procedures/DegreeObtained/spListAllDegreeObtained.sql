...
Create PROCEDURE [dbo].[spListAllDegreeObtained]	
...
AS
	...
	SELECT *
	INTO #TempTableFiltered FROM [DegreeObtained] 
...
AND (@userProfileId IS NULL OR UserProfileId = @userProfileId)
	...
	--set table total
	SELECT @TotalCount = ISNULL(COUNT(Id),0) FROM [DegreeObtained] WITH(NOLOCK) 
