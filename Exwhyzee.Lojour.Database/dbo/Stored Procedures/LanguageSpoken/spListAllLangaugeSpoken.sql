...
Create PROCEDURE [dbo].[spListAllLanguageSpoken]	
...
AS
	...
	SELECT *
	INTO #TempTableFiltered FROM [LanguageSpoken] 
...
AND (@userProfileId IS NULL OR UserProfileId = @userProfileId)
	...
	--set table total
	SELECT @TotalCount = ISNULL(COUNT(Id),0) FROM [LanguageSpoken] WITH(NOLOCK) 
