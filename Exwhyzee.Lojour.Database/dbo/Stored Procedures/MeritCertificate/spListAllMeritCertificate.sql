...
Create PROCEDURE [dbo].[spListAllMeritCertificate]	
...
AS
	...
	SELECT *
	INTO #TempTableFiltered FROM [MeritCertificate] 
...
AND (@userProfileId IS NULL OR UserProfileId = @userProfileId)
	...
	--set table total
	SELECT @TotalCount = ISNULL(COUNT(Id),0) FROM [MeritCertificate] WITH(NOLOCK) 
