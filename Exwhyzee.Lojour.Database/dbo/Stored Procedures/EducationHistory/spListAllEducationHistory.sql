...
Create PROCEDURE [dbo].[spListAllEducationHistory]	
...
AS
	...
	SELECT *
	INTO #TempTableFiltered FROM [EducationHistory] 

	
	WHERE 1 = 1 
AND (@userProfileId IS NULL OR UserProfileId = @userProfileId)
	--set table total
	SELECT @TotalCount = ISNULL(COUNT(Id),0) FROM [EducationHistory] WITH(NOLOCK) 
