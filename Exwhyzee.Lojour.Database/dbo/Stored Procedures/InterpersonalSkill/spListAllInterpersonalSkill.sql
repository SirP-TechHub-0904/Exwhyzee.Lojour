...
Create PROCEDURE [dbo].[spListAllInterpersonalSkill]	
...
AS
	...
	SELECT *
	INTO #TempTableFiltered FROM [InterpersonalSkill] 
...
AND (@userProfileId IS NULL OR UserProfileId = @userProfileId)
	...
	--set table total
	SELECT @TotalCount = ISNULL(COUNT(Id),0) FROM [InterpersonalSkill] WITH(NOLOCK) 
