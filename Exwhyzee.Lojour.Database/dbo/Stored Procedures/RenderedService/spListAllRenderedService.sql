...
Create PROCEDURE [dbo].[spListAllRenderedService]	
...
AS
	...
	SELECT *
	INTO #TempTableFiltered FROM [RenderedService] 
...
AND (@userProfileId IS NULL OR UserProfileId = @userProfileId)
	...
	--set table total
	SELECT @TotalCount = ISNULL(COUNT(Id),0) FROM [RenderedService] WITH(NOLOCK) 
